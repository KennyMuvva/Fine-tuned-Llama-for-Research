...
def process_pdf(pdf_path, output_dir):
    ...
    if os.path.exists(output_file_path):
        ...
        return
    print(f"Starting Extraction Pipeline: {pdf_name}")
...
def main():
    if not os.path.exists(OUTPUT_FOLDER):
        ...
        print(f"Created dynamic save directory: {OUTPUT_FOLDER}")
    if not os.path.exists(INPUT_FOLDER):
        print(f"Error: Specified input location directory missing: {INPUT_FOLDER}")
        return
    pdf_files = [os.path.join(INPUT_FOLDER, f) for f in os.listdir(INPUT_FOLDER) if f.lower().endswith('.pdf')]
    print(f"Discovered {len(pdf_files)} candidate PDFs inside processing library stack.")
    for path in pdf_files:
        process_pdf(path, OUTPUT_FOLDER)
    print("\nBatch automated data processing tasks finalized successfully.")
